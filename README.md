...
1. **管理员权限** —— 多数游戏以管理员运行，普通权限的工具无法向它注入按键。
   请用 `启动游戏快捷语录.bat` 启动（会自动请求管理员权限，UAC 点「是」）；
2. **先在记事本测试** —— 开记事本点一下输入框，按热键：有提示音且自动出现语录
   = 工具正常，问题在游戏侧；
3. **窗口模式** —— 游戏改成「无边框窗口化」比全屏独占兼容性好；
4. **聊天框按键** —— 确认游戏内按哪个键弹出聊天输入框，填到「打开聊天框按键」；
5. **热键冲突** —— 建议 Ctrl / Alt / Shift + F 键组合，避免与游戏按键冲突。

**没有提示音？** 检查系统是否静音/主板蜂鸣器是否可用（提示音用系统蜂鸣器播放）。
